...
export const booksRef = () =>
  collection(getFirestore(), 'books').withConverter(bookConverter);
...
export const bookStorageRef = (id: string) =>
  ref(getStorage(), `books/${id}`);

export const bookImageStorageRef = (id: string) =>
  ref(bookStorageRef(id), 'image');
...
export const createBook = async (
  ...
  data: Omit<BookDocumentData, 'createdAt' | 'updatedAt' | 'image'> & {
    image: File;
  },
) => {
  const { id } = ref;
  const { image, ...rest } = data;
  const { snapshot, url } = await uploadImage(
    bookImageStorageRef(id),
    image,
  );
  return setDoc(ref, {
    ...rest,
...
export const updateBook = async (
  ...
  const { id } = ref;
  const { image, ...rest } = data;
  const { snapshot, url } = image
    ? await uploadImage(bookImageStorageRef(id), image)
    : { snapshot: null, url: null };
